...
pub enum Error {
    ...
    KafkaSansIo(#[from] nisshi_sans_io::Error),
...
/// Generate a record
pub trait Generator {
    fn generate(&self) -> Result<nisshi_sans_io::record::Builder>;
}
...
mod tests {
    ...
    use crate::Result;
    use bytes::Bytes;
    use nisshi_sans_io::record::Record;
    ...
    fn error_size_of() -> Result<()> {
        ...
        debug!(sans_io = size_of::<nisshi_sans_io::Error>());
